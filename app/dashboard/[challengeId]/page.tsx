...
async function loadBundle(userId: string, challengeId: string): Promise<ChallengeDetailBundle | null> {
  try {
    ...
    const [firm, preset, snaps, trades, events] = await Promise.all([
      ...
      sb.from("equity_snapshots").select("*").eq("challenge_id", challengeId).order("timestamp", { ascending: true }).limit(500).catch(() => ({ data: [] })),
      sb.from("trades").select("*").eq("challenge_id", challengeId).order("open_time", { ascending: false }).limit(200).catch(() => ({ data: [] })),
      sb.from("rule_events").select("*").eq("challenge_id", challengeId).order("created_at", { ascending: false }).limit(200).catch(() => ({ data: [] })),
    ]);
